...
<?php 
            //If no values
            $amount = mysqli_query($conn,"SELECT Amount FROM Actuals WHERE `Week_quarter_ID` = '$date' AND Type_ID = '$type' AND Admin_ID = '$id'");
                $row_cnt = $amount->num_rows;
                    if($row_cnt == 0){ ?>
                        ...
                        <?php
                        header( "Refresh:2; url=inputActuals.php"); 
                    }else{ ?>

        <!-- Stuff In box -->
        <div class = "rcorners">
            <h1> Continue Editing Actuals</h1>
                <form action = "editActualsCheck.php" method = "post">
                    ...
                    <input type = "hidden" name = "date" id = "date" value = <?php echo $date ?> readonly></br>
                    <input type="submit">
                </form> 
            <br><br><br><br><a href = "editActuals.php"> Back </a>
        </div>
