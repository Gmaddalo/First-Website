...
if($expense == "Headcount"){
    ...
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, Actuals.Amount FROM Actuals JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Actuals.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Actuals.Type_ID) WHERE Actuals.Admin_ID = $id AND Expense_type_name = 'Headcount' AND Week_in_quarters.Quarter_ID = '$quarterid'")){
...
<!--Title -->
<h1 align = "center"> Headcount Actuals </h1>
<!-- Chart -->
...
//Discretionary Expenses
    ...
    //SQL statment
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, Actuals.Amount FROM Actuals JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Actuals.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Actuals.Type_ID) WHERE Actuals.Admin_ID = $id AND Expense_type_name = 'Discretionary' AND Week_in_quarters.Quarter_ID = '$quarterid'")){
...
<!-- Title -->
<h1 align ="center"> Discretionary Actuals </h1>
<!-- Chart -->
<div id="curve_chart"></div>
...
// Controllable
    ...
    //SQL statement
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, Actuals.Amount FROM Actuals JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Actuals.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Actuals.Type_ID) WHERE Actuals.Admin_ID = $id AND Expense_type_name = 'Controllable' AND Week_in_quarters.Quarter_ID = '$quarterid' ")){
...
<!-- Title --> 
<h1 align ="center"> Controllable Actuals </h1>
<!-- Chart -->
<div id="curve_chart"></div>
...
//Outsource expenses
    ...
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, Actuals.Amount FROM Actuals JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Actuals.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Actuals.Type_ID) WHERE Actuals.Admin_ID = $id AND Expense_type_name = 'Outsource' AND Week_in_quarters.Quarter_ID = '$quarterid'")){
...
<!-- Title -->
<h1 align ="center"> Outsource Actuals </h1>
<!-- Chart -->
<div id="curve_chart"></div>
...
//Totla
    ...
    //Select Statement
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Actuals.Amount) AS Amount FROM Actuals JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Actuals.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Actuals.Type_ID) WHERE Actuals.Admin_ID = '$id' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Actuals.Week_quarter_ID = Actuals.Week_quarter_ID GROUP BY Actuals.Week_quarter_ID")){
...
<!-- Title -->
<h1 align ="center"> Total for Actuals </h1>
<!-- Chart -->
<div id="curve_chart"></div>
...
?>
         <!-- Button -->
            <div align = "center">
                <div class = "btn">
                    <div class = "links2">
                        <a href = "chooseActualsGraph.php"> Back </a>
                    </div>
                </div></br> </br></br></br>
            </div>  
