...
<?php
    //get amount
    $amount = mysqli_query($conn,"SELECT Amount FROM Budget WHERE `Week_quarter_ID` = '$date' AND Type_ID = '$type' AND Admin_ID = '$id' ");
        $row_cnt = $amount->num_rows;
            ...
            if($row_cnt == 0)
                { ?>
                    ...
                    <?php
                        header( "Refresh:2; url=inputBudget.php");               
                }else{ ?>
        ...
        <div class ="rcorners">
            <h1> Do you want to delete this?</h1>
                <form action = "deleteBudgetsCheck.php" method = "post" onsubmit="required()">
