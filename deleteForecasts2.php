...
<?php
    $amount = mysqli_query($conn,"SELECT Amount FROM Year_Forecast WHERE `Week_quarter_ID` = '$date' AND Type_ID = '$type' AND Admin_ID = '$id' ");
                ...
                if($row_cnt == 0)
                    {   
                        ...
                        <div class = "rcorners">
                            ...
                            <h1> Please Create a Forecast</h1>
                        </div>
                        <?php
                        header( "Refresh:2; url=createForecast.php");
                    }else{ ?>

        <div class ="rcorners">
            <h1> Do you want to delete this?</h1>
                <form action = "deleteForecastsCheck.php" method = "post" onsubmit="required()">
                    Amount:<input type="number" name="newamount" id = "newamount" value = 
                        <?php
                            $amount = mysqli_query($conn,"SELECT Amount FROM Year_Forecast WHERE `Week_quarter_ID` = '$date' AND Type_ID = '$type' AND Admin_ID = '$id' ");
                            $row_cnt = $amount->num_rows;
                            // if($row_cnt == 0)
            ...
            <a href = "deleteBudgets.php"> Back </a>
        </div>
