...
<?php
// For headcount Expense
if($expense == "Headcount"){

    //Sql Statement 
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Expenses.Amount) AS Amount_Sum FROM Expenses JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Expenses.       Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Expenses.Type_ID) WHERE Expenses.Admin_ID = '$id' AND Expense_type_name = 'Headcount' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Expenses.Week_quarter_ID = Expenses.Week_quarter_ID GROUP BY Expenses.Week_quarter_ID")){
...
?>
    ...
    <!-- Graph Name -->
    <h1 align = "center"> Headcount Expenses (USD) </h1>
...
//For Discretionary
    ...
    //Sql statement
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Expenses.Amount) AS Amount_Sum FROM Expenses JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Expenses.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Expenses.Type_ID) WHERE Expenses.Admin_ID = '$id' AND Expense_type_name = 'Discretionary' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Expenses.Week_quarter_ID = Expenses.Week_quarter_ID GROUP BY Expenses.Week_quarter_ID  ")){
...
<!-- Graph Name -->
<h1 align = "center"> Discretionary Expenses (USD) </h1>
...
// For controllable
    ...
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Expenses.Amount) AS Amount_Sum FROM Expenses JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Expenses.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Expenses.Type_ID) WHERE Expenses.Admin_ID = '$id' AND Expense_type_name = 'Controllable' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Expenses.Week_quarter_ID = Expenses.Week_quarter_ID GROUP BY Expenses.Week_quarter_ID ")){
...
<!-- Graph Name -->
<h1 align = "center"> Controllable Expenses (USD) </h1>
<!-- Creating Chart -->
<div id="curve_chart"></div>
...
//Outsource
    ...
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Expenses.Amount) AS Amount_Sum FROM Expenses JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Expenses.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Expenses.Type_ID) WHERE Expenses.Admin_ID = '$id' AND Expense_type_name = 'Outsource' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Expenses.Week_quarter_ID = Expenses.Week_quarter_ID GROUP BY Expenses.Week_quarter_ID ")){
...
<!-- Graph Name -->
<h1 align = "center"> Outsource Expenses (USD) </h1>
<!--Creating Chart-->
<div id="curve_chart"></div>
...
//Total
    ...
    //Sql statment with Sum of everything
    if($stmt = $conn->query("SELECT Week_in_quarters.Day_due, SUM(Expenses.Amount) AS Amount_Sum FROM Expenses JOIN Week_in_quarters ON (Week_in_quarters.Week_quarter_ID = Expenses.Week_quarter_ID) JOIN Expense_Type ON(Expense_Type.Type_ID = Expenses.Type_ID) WHERE Expenses.Admin_ID = '$id' AND Week_in_quarters.Quarter_ID = '$quarterid' AND Expenses.Week_quarter_ID = Expenses.Week_quarter_ID GROUP BY Expenses.Week_quarter_ID ")){
...
<!-- Graph Name -->
<h1 align = "center"> Total Expenses (USD) </h1>
<!-- Create Chart -->
<div id="curve_chart"></div>
...
?>
            ...
            <div align = "center">
                <div class = "btn">
                    <div class = "links2">
                        <a href = "chooseExpensesGraph.php"> Back </a>
                    </div>
                </div></br> </br></br></br>
            </div>  
